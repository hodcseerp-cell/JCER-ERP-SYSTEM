...
const AuthLayout = () => {
    ...
    return (
        <div className="admission-portal-theme min-h-screen w-full bg-slate-50 font-display flex flex-col relative">
            ...
            {/* Main Content Area */}
            <main className="flex-1 w-full p-3 sm:p-5 lg:p-6 flex flex-col justify-start items-center relative z-10">
                {/* Top Control Bar with Back to Home Button */}
                <div className="w-full max-w-[1050px] flex items-center justify-between mb-2 sm:mb-3 shrink-0">
                    <button
                        type="button"
                        onClick={() => {
                ...
                <div className="group w-full max-w-[1050px] grid grid-cols-1 lg:grid-cols-2 items-stretch bg-white rounded-3xl shadow-2xl hover:shadow-[0_30px_70px_-15px_rgba(37,99,235,0.22)] border border-slate-200/80 hover:border-primary-400/60 h-auto animate-fade-in text-slate-900 my-auto transition-all duration-500 ease-out hover:-translate-y-1 overflow-hidden">
                    ...
                    <div className="hidden lg:flex relative bg-slate-900 border-r border-slate-200/80 h-full flex-col justify-end rounded-l-3xl overflow-hidden min-h-[420px]">
                        ...
                        {/* Content Overlay */}
                        <div className="relative h-full flex flex-col justify-end p-6 lg:p-7 xl:p-8 text-white z-10">
                            <div className="mb-4 xl:mb-6 space-y-2 xl:space-y-2.5">
                                <span className="bg-white/20 backdrop-blur-lg px-3.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-[0.2em] mb-2 inline-block border border-white/25 shadow-sm group-hover:bg-primary-600/30 group-hover:border-primary-400/50 transition-all duration-300">
                                    Welcome Back
                                </span>
                                <h1 className="text-2xl lg:text-3xl xl:text-4xl font-extrabold leading-[1.15] mb-2 text-shadow-premium">
                                    Empowering Your <br />
                                    <span className="text-primary-300 group-hover:text-primary-200 transition-colors">Academic Journey.</span>
                                </h1>
                                <p className="text-white/80 text-xs lg:text-sm leading-relaxed max-w-md font-medium">
                                    Access your academic records, course registrations, and institutional resources in one secure portal.
                                </p>
                            </div>
                            
                            <div className="flex items-center gap-4 pt-3 xl:pt-5 border-t border-white/15">
                                <div className="flex -space-x-2.5">
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className="size-8 sm:size-9 rounded-full border-2 border-slate-900 bg-slate-800 overflow-hidden ring-2 ring-white/10 group-hover:ring-primary-400/30 transition-all">
                                            <img 
                    ...
                    <div className="p-5 sm:p-7 lg:p-7 xl:p-8 flex flex-col justify-center bg-white relative z-10 h-full rounded-r-3xl lg:rounded-l-none rounded-3xl">
                        <Outlet />
                    </div>
                </div>
            </main>
